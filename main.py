...
logdir = 'D:\chrome download\srgan-master\srgan-master\log'
...
#     valid_hr_img_list = sorted(tl.files.load_file_list(path=VALID.hr_img_path, regx='.*.png', printable=False))
#     valid_lr_img_list = sorted(tl.files.load_file_list(path=VALID.lr_img_path, regx='.*.png', printable=False))

    ## If your machine have enough memory, please pre-load the whole train set.
    train_hr_imgs = tl.vis.read_images(train_hr_img_list, path=config.TRAIN.hr_img_path, n_threads=32)
    # for im in train_hr_imgs:
    #     print(im.shape)
    # valid_lr_imgs = tl.vis.read_images(valid_lr_img_list, path=VALID.lr_img_path, n_threads=32)
    # for im in valid_lr_imgs:
    #     print(im.shape)
    # valid_hr_imgs = tl.vis.read_images(valid_hr_img_list, path=VALID.hr_img_path, n_threads=32)
    ...
    print(train_hr_imgs)
    print(sample_imgs)
#     sample_imgs = tl.vis.read_images(train_hr_img_list[0:batch_size], path=hr_img_path, n_threads=32) # if no pre-load train set
...
#     tl.vis.save_images(sample_imgs_384, [ni, ni], save_dir_gan + '/_train_sample_384.png')
    ...
    for epoch in range(0, n_epoch_init + 1):
        ...
        writer = SummaryWriter(logdir)  # 定义一个tensorboardX的写对象
        writer.add_scalars(logdir + 'total_mse_loss',{logdir + 'total_mse_loss':total_mse_loss},epoch)
...
def evaluate():
    ...
    valid_lr_img = get_imgs_fn('input2.jpg',path='D:\chrome download\SRCNN-keras-master\\')  # if you want to test your own image
    ...
    sess = tf.Session(config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=False))
    tl.layers.initialize_global_variables(sess)
    tl.files.load_and_assign_npz(sess=sess, name='D:\chrome download\srgan-master\srgan-master\checkpoint\g_srgan.npz', network=net_g)
...
if __name__ == '__main__':
    ...
    #
    # parser.add_argument('--mode', type=str, default='srgan-master', help='srgan-master, evaluate')
    #
    # args = parser.parse_args()
    #
    # 'srgan-master' = args.mode
    #
    # if 'srgan-master' == 'srgan-master':
    #     train()
    # elif 'srgan-master' == 'evaluate':
    #     evaluate()
    # else:
